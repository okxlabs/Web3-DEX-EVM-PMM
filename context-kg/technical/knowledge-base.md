...
tags: ["knowledge-base", "hard-rules", "CallerAuth", "caller-binding", "allowedSender", "version-1.2", "orderType-4", "anti-toxic-flow", "SCDEX-1157"]
...
## 0. Anti-Toxic-Flow / Caller Binding (SCDEX-1157) — NEW hard rules

- [Rule] The settlement entry is the **single** `fillOrderRFQTo(order, signature, flagsAndAmount, target, address[] allowedCallers, uint256 nonce, uint256 expiry, bytes okxSig)`. The prior fill variants (`fillOrderRFQ`, `fillOrderRFQCompact`, `fillOrderRFQToWithPermit`) are **removed** — never reference them as live entrypoints.
- [Rule] `fillOrderRFQTo`'s **first line** is `_verifyCallerAuth(allowedCallers, nonce, expiry, okxSig)` (OKX caller binding). Never reorder it after the maker-signature check or fund movement.
...
- [Rule] `OKX_SIGNER` is `immutable`, set in the constructor of both `PMMProtocol` and `PMMAdapter`; a zero signer is rejected at deploy (`OSA_ZeroSigner`). Never add a setter.
- [Rule] Caller-auth uses an append-only Permit2-style nonce bitmap (`_callerAuthNonceBitmap`, storage slot 0 in both contracts); the nonce is consumed in `_verify*` before any external call (CEI). `okxSig` must be EIP-2098 64-byte compact (else `OSA_BadSigLen`).
- [Rule] `_extractDexRouterCaller` reads calldata `-64` with an **exact** marker match (`word & MARKER_MASK == DEX_ROUTER_CALLER_MARKER`) and fail-closes to `address(0)`; the refund word stays at `-32` (`ORIGIN_PAYER`). Never conflate the two markers.
- [Rule] Legacy `orderType ∈ {1,2,3}` adapter paths are unchanged and must stay caller-auth-free / allowedSender-free (backward compat, FR-6-AC-1).
...
- [Rule] `_NAME = "OKX Labs PMM Protocol"` and `_VERSION = "1.2"` constants in `PmmProtocol.sol` are bound into the cached domain separator at construction — changing either invalidates every outstanding maker signature. The `1.1 → 1.2` bump (SCDEX-1157, for the `allowedSender` field) already invalidated all `1.1` signatures.
...
| `contracts/contract-CallerAuth.md` | OKX caller-binding base — allowedCallers, nonce bitmap, dexRouterCaller (SCDEX-1157) |
