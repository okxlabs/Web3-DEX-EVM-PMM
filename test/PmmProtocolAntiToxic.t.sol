...
contract PmmProtocolAntiToxicTest is Test {
    ...
    // A forged adaptor authorization signed by the wrong key -> AUTH_BadAuthSig.
    function testAdapterForgedAuthSigReverts() public {
        ...
        uint256 nonce = _nextNonce();
        CallerAuthData memory adaptorAuth = CallerAuthData({
    ...
    // The protocol also rejects an authorization whose signed allowedCallers list the caller but is
    // signed by the wrong key -> AUTH_BadAuthSig.
    function testProtocolForgedAuthReverts() public {
        (, uint256 wrongKey) = makeAddrAndKey("notAuthSigner");
        OrderRFQLib.OrderRFQ memory order = _order(dexUser);
        bytes memory orderSig = _signOrder(order);
        ...
        bytes memory forgedSig = _signAuth(address(protocol), keccak256(abi.encode(order)), callers, nonce, wrongKey);

        vm.prank(caller);
        vm.expectRevert(CallerAuth.AUTH_BadAuthSig.selector);
        protocol.fillOrderRFQTo(order, orderSig, 0, recipient, callers, nonce, forgedSig);
    }
